...
type fakeAPI struct {
	...
	execFunc    func(args ...string) ([]byte, string, error)
}
...
func (f *fakeAPI) Exec(args ...string) ([]byte, string, error) {
	if f.execFunc == nil {
...
func TestServiceStart(t *testing.T) {
	...
	api.graphqlFunc = func(query string, variables map[string]interface{}, result interface{}) error {
		gqlCall++
		switch gqlCall {
		case 1: // currentViewer
			assert.Contains(t, query, "ViewerLogin")
	...
	api.execFunc = func(args ...string) ([]byte, string, error) {
		assert.Equal(t, []string{"pr", "view", "7", "--json", "comments,reviews"}, args)
		return []byte(`{"comments":[], "reviews":[]}`), "", nil
	}
...
func TestServiceStartErrorOnEmptyState(t *testing.T) {
	...
	pr := resolver.Identity{Owner: "octo", Repo: "demo", Number: 7, Host: "github.com"}
	_, err := svc.Start(pr, "")
	require.NoError(t, err) // No longer fails on Start
}
